# Вам доступен текстовый файл nums.txt. В файле могут быть записаны целые неотрицательные числа и все, что угодно. Числом назовем последовательность
...
with open("nums.txt", "r", encoding="utf-8") as nums:
    contents = nums.read()
    number = re.findall(r'\d+', contents)
    total = 0
    for num in number:
        total += int(num)
